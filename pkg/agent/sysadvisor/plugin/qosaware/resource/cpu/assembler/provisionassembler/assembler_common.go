...
//  5. assembleEmptyNUMARegion           – allocate reclaim pool with CFS quota for NUMAs with no real workloads
//
...
// assembleEmptyNUMARegion handles empty NUMA regions on NUMA nodes that have no real workload regions.
// It retrieves the provision control knob (ControlKnobReclaimedCoresCPUQuota) from each empty region,
...
func (pa *ProvisionAssemblerCommon) assembleEmptyNUMARegion(regionHelper *RegionMapHelper, result *types.InternalCPUCalculationResult) error {
	for numaID := range *pa.numaAvailable {
		emptyNUMARegions := regionHelper.GetRegionsByNUMA(numaID, configapi.QoSRegionEmptyNUMA)
